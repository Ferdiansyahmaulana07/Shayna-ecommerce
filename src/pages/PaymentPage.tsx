...
import { type BookingFormData, type CartItem, type Cosmetic } from "../types/type";
...
//Define the type of the formdata
type formData = {
    proof: File | null;
    cosmetic_ids: { id: number; quantity: number}[]; //Upload to incluede quantity
 };
...
export default function PaymentPage() {
    ...
    const [succesMessage, setSuccesMesagge] = useState<string | null>(null);
    ...
    const fetchCosmeticDetails = async(cartItems: CartItem[]) => {
        try {
            ...
            //Map each cart item to incluede both `id` and `quantity`
        ...
        const handleSubmit= async (e: React.FormEvent) => {
          ...
          try {
            ...
            if (response.status === 200 || response.status === 201) {
              console.log("Transaction responsedata:", response.data.data);//Check response content of response data
              ...
              setSuccesMesagge("Payment proof uploaded succesfully!");
